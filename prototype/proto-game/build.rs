...
fn main() {
    let params = RomDataSpec {
        version: (0, 1, 16),
        gfx: entry_vec!(("example", "assets/gfx/example.data")),
    };
    ...
    template.raw_line(
        format!(
            "fn {}<F, T>(func: F) -> {}",
            FN_CALC_ROM_DATA_OFFSET, TYPE_ROM_DATA_POINTER
        )
        .as_str(),
    );
    ...
    template.raw_line(
        format!(
            "    func(rom_data()) as *const T as {}",
            TYPE_ROM_DATA_POINTER
        )
        .as_str(),
    );
    template.raw_line("}");
    ...
    } in &params.gfx
    {
        template.field(
            id,
            format!("{}<[u8; {}usize]>", TYPE_ROM_DATA_ENTRY, byte_count).as_str(),
        );
    }
    template.struct_end();
    ...
    } in &params.gfx
    {
        ...
        template.raw_line(
            format!(
                "            {}(|rom| &rom.{}.{}),",
                FN_CALC_ROM_DATA_OFFSET, FIELD_ROM_DATA_GFX, id
            )
            .as_str(),
        );
    ...
    template.raw_line(
        format!(
            "            {}: {} {{",
            FIELD_ROM_DATA_GFX, TYPE_ROM_DATA_GFX
        )
        .as_str(),
    );
    for RomDataEntrySpec {
